...
The local smoke command starts Wrangler, both Durable Objects, and a real Docker-backed Sandbox container itself, then removes its temporary state. It covers enrollment, heartbeat, capability probing, every file action, a non-root command, symlink containment, byte limits, action replay and conflict, lease expiry, authentication failure, disposal, revocation, and enrollment reuse. Omit `GROKKY_GATEWAY_SMOKE_SKIP_BROWSER=1` to include the local Browser Run emulator; if Cloudflare's browser download stalls, keep the skip explicit and prove Browser Run against the deployed gateway with root `npm run smoke:cloud-device` from a paired installed app. The authoritative runbook also documents the ephemeral one-time-enrollment mode for unattended release verification.
