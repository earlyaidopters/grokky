...
export function PhoneControl({ conversationId, status, build, onError }: { conversationId: string; status?: PhoneDesktopStatus; build?: string; onError(message: string): void }) {
  ...
  useEffect(() => {
    ...
    if (session?.inviteUrl) void QRCode.toDataURL(session.inviteUrl, { width: 216, margin: 2 }).then((url) => { if (!cancelled) setQr(url); }).catch(() => setError("Could not display the pairing code. You can still copy the pairing link."));
  ...
  useEffect(() => { setError(""); setFeedback(""); }, [conversationId]);
  const act = async (label: string, action: () => Promise<void>, success = "") => {
    ...
    catch (e) { const message = e instanceof Error ? e.message : "Phone control failed. Try again."; setError(message); onError(message); }
  ...
  return <section className="phone-control" aria-busy={Boolean(busy)}>
    ...
    {expanded && <div className="phone-control-content">
      {!session && <><p>Watch, approve an action, or take over this cloud browser from your phone.</p>{status && <p>Another conversation is paired. Disconnect that phone session before pairing this task.</p>}<button type="button" disabled={Boolean(busy) || Boolean(status)} onClick={() => void act("Preparing pairing…", () => window.grokky.startPhone(conversationId))}>Pair phone</button></>}
